...
module.exports = function container (get, set, clear) {
  ...
  return function (s) {
    ...
    var max_fa_width = 0
    function fa (amt, omit_currency, color_trick, do_pad) {
      var str
      var fstr
      amt > 999 ? fstr = '0.00' :
        amt > 99 ? fstr = '0.000' :
      ...
      if (do_pad) {
        max_fa_width = Math.max(max_fa_width, str.length)
        str = ' '.repeat(max_fa_width - str.length) + str
      }
      if (color_trick) {
        str = str
      ...
      return str + (omit_currency ? '' : ' ' + s.asset)
    }
      ...
      s.exchange.getBalance({currency: s.currency, asset: s.asset}, function (err, balance) {
        ...
        if (!s.start_capital) {
          s.exchange.getQuote({product_id: s.product_id}, function (err, quote) {
            ...
            pushMessage('Balance Start ' + s.exchange.name.toUpperCase(), '\n' + 'Asset Balance: '+ s.balance.asset + ' ' + s.asset + '\n' + 'Current Value: ' + s.start_capital+'\n'+ 'Available Currency:' +s.balance.currency + ' ' + s.currency  )
    ...
    function reportOut () {
      syncBalance(function (err) {
        ...
        if (s.period) {
          ...
          b.profit = (b.consolidated - s.start_capital) / s.start_capital
          b.buy_hold = s.period.close * (s.start_capital / s.start_price)
          b.buy_hold_profit = (b.buy_hold - s.start_capital) / s.start_capital
      ...
      var consolidated = n(s.balance.asset).multiply(s.period.close).add(s.balance.currency).value()
      var profit = (consolidated - s.start_capital) / s.start_capital
      var buy_hold = s.period.close * (s.start_capital / s.start_price)
      var vs_buy_hold = (consolidated - buy_hold) / buy_hold
      pushMessage('Update ' + s.exchange.name.toUpperCase() +
        '\nAsset Balance: '+ fa(s.balance.asset) +
        '\nCurrent Value: ' + s.start_capital +
        '\nAvailable Currency:' + fc(s.balance.currency) +
        '\nNumber or Trades: ' + s.my_trades.length +
        '\nLast Trade Worth: '+ fc(s.last_trade_worth) +
        '\nProfit for run: ' + pct(profit) +
        '\nVS Buy & Hold: ' + pct(vs_buy_hold) +
        '\nStart Price: ' + fc(s.start_price) +
        '\nCurrent Price: ' + fc(s.period.close))
    }
          ...
          if (signal === 'buy') {
            ...
            if ((s.product.min_size && Number(size) >= Number(s.product.min_size)) || ('min_total' in s.product && s.product.min_total && n(size).multiply(price).value() >= Number(s.product.min_total))) {
              ...
              if (n(s.balance.currency).subtract(s.balance.currency_hold || 0).value() < n(price).multiply(size).value() && s.balance.currency_hold > 0) {
                ...
                pushMessage('Buying ' + s.exchange.name.toUpperCase(), 'placing buy order at ' + fc(price) + ' for ' + fa(size) + '\n' + 'Total: ' + fc(price*size)+'\n')
                doOrder()
              }
            }
            else {
            ...
            if ((s.product.min_size && Number(size) >= Number(s.product.min_size)) || (s.product.min_total && n(size).multiply(price).value() >= Number(s.product.min_total))) {
              ...
              if (so.max_sell_loss_pct && sell_loss > so.max_sell_loss_pct) {
                ...
                if (n(s.balance.asset).subtract(s.balance.asset_hold || 0).value() < n(size).value()) {
                  ...
                  pushMessage('Selling ' + s.exchange.name.toUpperCase(), 'placing sell order at ' + fc(price) + ' for ' + fa(size) + '\n' + fc(price*size)+'\n')
                  doOrder()
                }
              }
            }
